...
class UnetGenerator(nn.Module):
    def __init__(self, input_nc, output_nc, num_downs, ngf=64,
                 norm_layer=nn.BatchNorm2d, use_dropout=False, use_cuda=False):
...
class VGGLoss(nn.Module):
    def __init__(self, layids=None, use_cuda=False):
...
class GMM(nn.Module):
    ...
    def __init__(self, hight, weight, grid_size, use_cuda=False):
        ...
        self.gridGen = TpsGridGen(hight, weight, use_cuda=use_cuda, grid_size=grid_size)
...
def make_image(key_points, shape):
    """
    ...
    r = 5
    im_pose = Image.new('L', (shape[1], shape[0]))
    ...
    pose_map = torch.zeros(point_num, shape[0], shape[1])
    for i in range(point_num):
        one_map = Image.new('L', (shape[1], shape[0]))
...
class Viton():
    ...
    def __init__(self, weight=192, hight=256, grid_size=5,
                 gmm_checkpoint_path='checkpoint', tom_checkpoint_path='checkpoint', use_cuda=False):

        self.gmm = GMM(hight, weight, grid_size, use_cuda)
    ...
    def run_viton(self, head, pose_map, shape, cloth, cloth_mask):
        agnostic = self.get_agnostic(shape.type(torch.float), head.type(torch.float), pose_map.type(torch.float))
        print('LOLOLAGN: ', agnostic.shape)
